...
func test_file_creation() -> void:
	...
	var log_path = Logger._get_log_path()
...
func test_log_rotation() -> void:
	...
	for i in Logger.MAX_LOG_FILES + 1:
		Logger.info("Test log rotation %d" % i)
	...
	assert(log_count <= Logger.MAX_LOG_FILES, "Should not exceed max log files")
	print("✓ Log rotation keeps <= %d files" % Logger.MAX_LOG_FILES)
